...
<article class="glass group flex w-full flex-col overflow-hidden rounded-2xl transition-all duration-300 hover:-translate-y-1 hover:glow-gold">
	...
	<h3 class="order-1 line-clamp-2 min-h-[2.25rem] px-3 pt-3 text-xs font-bold leading-relaxed text-ink lg:order-2 lg:min-h-[2.5rem] lg:px-4 lg:pt-4 lg:text-sm">
		<a href="<?php echo esc_url( $permalink ); ?>" class="transition-colors hover:text-gold">
			<?php echo esc_html( $title ); ?>
		</a>
	</h3>

	<!-- تصویر محصول مربعی با افکت زوم و هاله‌ی تاریک روی هاور — موبایل order-2 → دسکتاپ order-1 -->
	<a href="<?php echo esc_url( $permalink ); ?>" class="relative order-2 block aspect-square w-full overflow-hidden bg-surface lg:order-1">
		...
		<div class="absolute bottom-2 left-2 z-10 flex gap-1">
			<?php if ( $romanino_fmt['has_pdf'] ) : ?>
				<span class="flex h-5 items-center gap-1 rounded-md bg-[#0b0514]/90 px-1.5 text-[9px] font-bold text-ink-2 backdrop-blur-sm" title="نسخه PDF موجود است">📄 PDF</span>
			<?php endif; ?>
			<?php if ( $romanino_fmt['has_audio'] ) : ?>
				<span class="flex h-5 items-center gap-1 rounded-md bg-[#0b0514]/90 px-1.5 text-[9px] font-bold text-ink-2 backdrop-blur-sm" title="نسخه صوتی موجود است">🎧 صوتی</span>
			<?php endif; ?>
		</div>
	</a>
	...
	<div class="order-3 flex flex-1 flex-col px-3 pb-3 lg:order-3 lg:p-4 lg:pt-0">
		...
		<div class="mt-1 flex items-center justify-between gap-1.5">
			<p class="truncate text-[11px] text-ink-muted lg:text-xs"><?php echo esc_html( $byline_label ); ?></p>
			<span class="flex shrink-0 items-center gap-1 text-[10px] text-ink-faint lg:text-[11px]">
		...
		<div class="mt-auto pt-2.5 lg:pt-3">
			<!-- قیمت -->
			<div class="mb-2.5 flex items-center gap-2 text-xs font-bold text-gold lg:mb-3 lg:text-sm">
			...
			<?php else : ?>
				...
				<button type="button"
					class="romanino-buy-btn flex w-full items-center justify-center gap-1.5 rounded-xl bg-primary py-2.5 text-[11px] font-bold text-[#0f0726] transition-all duration-200 hover:brightness-110 hover:shadow-[0_0_15px_rgba(234,179,8,0.5)] lg:text-xs"
