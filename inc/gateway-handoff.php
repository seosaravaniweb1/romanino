...
function romanino_show_zibal_failure( $order_id, $fault = '' ): void {
	...
	$known = array(
		...
		/* تأییدشده توسط پشتیبانی زیبال: کد ۱۱۵ یعنی IP سروری که درخواست از آن
		   آمده، در پنل زیبال ثبت/تأیید نشده است. این کد در مستندات عمومی
		   زیبال نیامده بود، برای همین اینجا با متن کامل راه‌حل نوشته شده. */
		'115'        => 'IP سرور سایت در پنل زیبال ثبت نشده است. در پنل زیبال → «درگاه پرداخت» → تنظیمات درگاه (چرخ‌دنده) → «تنظیمات کلی»، IP هاست را ثبت کنید.',
	);
