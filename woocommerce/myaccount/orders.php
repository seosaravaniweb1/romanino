<?php
/**
 * My Account Orders — انتشارات سرو (فایل جدید)
...
<div>
	<div class="mb-6 flex items-center justify-between">
		<div>
			<h2 class="font-naskh text-lg font-bold text-teal">سفارش‌های من</h2>
			<p class="mt-1 text-xs text-muted-foreground">تاریخچه‌ی خریدهای شما از انتشارات سرو.</p>
		</div>
	</div>
	...
	<?php if ( ! $has_orders ) : ?>
		...
		<!-- حالت خالی -->
		<div class="flex flex-col items-center gap-4 rounded-2xl border border-gold-hair bg-card px-6 py-14 text-center">
			...
			<p class="max-w-xs text-xs leading-relaxed text-muted-foreground">
				بعد از خرید هر کتاب، سفارش شما همین‌جا نمایش داده می‌شود.
			</p>
	...
	<?php else : ?>
		...
		<div class="space-y-3">
			...
			?>
			<div class="flex flex-col gap-3 rounded-2xl border border-gold-hair bg-card p-4 sm:flex-row sm:items-center sm:justify-between">
				<div class="flex items-center gap-3">
					...
					<div>
						...
						<p class="mt-0.5 text-[11px] text-muted-foreground">
							...
							·
							<?php echo esc_html( sprintf( _n( '%s کالا', '%s کالا', $item_count, 'saro' ), $item_count ) ); ?>
						</p>
					</div>
				</div>
				...
				<div class="flex items-center gap-3 sm:gap-4">
					...
					<div class="flex items-center gap-2">
						...
						<a href="<?php echo esc_url( $order->get_view_order_url() ); ?>"
							class="rounded-lg border border-gold-hair px-3 py-1.5 text-xs font-bold text-foreground hover:bg-secondary">
							جزئیات
						</a>
					</div>
				</div>
		...
		<nav class="mt-8 flex items-center justify-center gap-2">
			...
			<a href="<?php echo esc_url( wc_get_endpoint_url( 'orders', $current_page - 1 ) ); ?>"
				class="rounded-lg border border-gold-hair px-3 py-1.5 text-xs font-semibold text-foreground hover:bg-secondary">&raquo; قبلی</a>
			<?php endif; ?>
			...
			<a href="<?php echo esc_url( wc_get_endpoint_url( 'orders', $current_page + 1 ) ); ?>"
				class="rounded-lg border border-gold-hair px-3 py-1.5 text-xs font-semibold text-foreground hover:bg-secondary">بعدی &laquo;</a>
			<?php endif; ?>
		</nav>
		<?php endif; ?>
